...
namespace AnotherRpgMod.Items
{
    class AdditionalProjectile : ItemNodeAdvanced
    {
        ...
        new public float rarityWeight = 0.005f;
        ...
        public override string GetDesc
        {
            get
            {
                return "+ " + (ProjectileAmmount * Utils.Mathf.Clamp(GetLevel, 1, GetMaxLevel)) + " Projectile with 30% damage";
            }
        }
        ...
        public override void OnShoot(Item item, Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
            for (int i = 0; i < ProjectileAmmount* GetLevel; i++)
            {
                ...
                int projnum = Projectile.NewProjectile(position.X, position.Y, spdX, spdY, type, (int)(damage*0.3f), knockBack, player.whoAmI);
        ...
        public override void SetPower(float value)
        {
            ...
            m_MaxLevel = 1;
            m_RequiredPoints = 1 + Utils.Mathf.FloorInt(value * 0.35f);
            power = value;
        }
