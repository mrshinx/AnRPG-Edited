    ...
    class NPCUtils
    {
        ...
        public static float UpdateDOT (NPC npc)
        {
            ...
            if (life > 1)
            {
                ...
                {
                    DoTDamage += Mathf.Clamp(  Mathf.Logx(npc.lifeMax,1.01f) * 0.25f * DELTATIME , 0 , npc.lifeMax*0.0001f * DELTATIME);
                }
                if (npc.HasBuff(BuffID.Burning))
                {
                    DoTDamage += Mathf.Clamp(Mathf.Logx(npc.lifeMax, 1.01f) * 0.05f * DELTATIME, 0, npc.lifeMax * 0.004f * DELTATIME);
                }
                if (npc.HasBuff(BuffID.Frostburn))
                {

                    DoTDamage += Mathf.Clamp(Mathf.Logx(npc.lifeMax, 1.01f) * 0.25f * DELTATIME, 0, npc.lifeMax * 0.0003f * DELTATIME);
                }
                if (npc.HasBuff(BuffID.Venom))
                {

                    DoTDamage += Mathf.Clamp(Mathf.Logx(npc.lifeMax, 1.01f) * 0.25f * DELTATIME, 0, npc.lifeMax * 0.0005f * DELTATIME);
                }
                if (npc.HasBuff(BuffID.CursedInferno))
                {
                    DoTDamage += Mathf.Clamp(  Mathf.Logx(npc.lifeMax,1.01f) * 0.25f * DELTATIME , 0 , npc.lifeMax*0.0007f * DELTATIME);
                }
            }
            return DoTDamage;
        }
